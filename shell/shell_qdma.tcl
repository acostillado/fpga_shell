...
proc instantiate_qdma { config_dict bd_instance_name } {

# Extract required variables from the configuration dictionary
set PCIEentry        [dict get $config_dict PCIEentry]
set PortList         [dict get $config_dict PortList]
set g_pcie_file      [dict get_default $config_dict g_pcie_file ""]
set pcieBlockLoc     [dict get $config_dict pcieBlockLoc]
...
dict set config_dict PortList $PortList
  ...
  set qdma_0 [ create_bd_cell -type ip -vlnv xilinx.com:ip:qdma $bd_instance_name ]
  set_property -dict [ list \
 ...
 # Create instance: util_ds_buf, and set properties
  ...
  #connect_bd_intf_net -intf_net qdma_0_M_AXI_LITE [get_bd_intf_pins axi_periph/S00_AXI] [get_bd_intf_pins $bd_instance_name/M_AXI_LITE]
  connect_bd_intf_net -intf_net qdma_0_pcie_mgt [get_bd_intf_ports pci_express_x16] [get_bd_intf_pins $bd_instance_name/pcie_mgt]
  connect_bd_net -net pcie_perstn_1 [get_bd_ports pcie_perstn] [get_bd_pins $bd_instance_name/soft_reset_n] [get_bd_pins $bd_instance_name/sys_rst_n]
 # connect_bd_net -net resetn_1 [get_bd_ports resetn] [get_bd_pins proc_sys_reset_0/ext_reset_in] [get_bd_pins rst_dvino_alveo_300M/ext_reset_in]
  connect_bd_net -net vdd_0_dout [get_bd_pins $bd_instance_name/qsts_out_rdy] [get_bd_pins $bd_instance_name/tm_dsc_sts_rdy] [get_bd_pins vdd_0/dout]
  connect_bd_net -net util_ds_buf_IBUF_DS_ODIV2 [get_bd_pins $bd_instance_name/sys_clk] [get_bd_pins util_ds_buf/IBUF_DS_ODIV2]
  connect_bd_net -net util_ds_buf_IBUF_OUT [get_bd_pins $bd_instance_name/sys_clk_gt] [get_bd_pins util_ds_buf/IBUF_OUT]

  create_bd_cell -type ip -vlnv xilinx.com:ip:proc_sys_reset:5.0 proc_sys_rst_pcie

  connect_bd_net [get_bd_pins $bd_instance_name/axi_aclk] [get_bd_pins proc_sys_rst_pcie/slowest_sync_clk]
  connect_bd_net [get_bd_pins $bd_instance_name/phy_ready] [get_bd_pins proc_sys_rst_pcie/dcm_locked]
  connect_bd_net [get_bd_pins $bd_instance_name/axi_aresetn] [get_bd_pins proc_sys_rst_pcie/ext_reset_in]
   ...
   if { $PCIeDMA != "dma" } {
	   ...
	   make_bd_intf_pins_external  [get_bd_intf_pins $bd_instance_name/M_AXI]
	   set_property name $PCIEifname [get_bd_intf_ports M_AXI_0]
	   #Clock and reset
	   make_bd_pins_external  [get_bd_pins $bd_instance_name/axi_aclk]
	   make_bd_pins_external  [get_bd_pins $bd_instance_name/axi_aresetn]
	...
	set PCIeDMAdone 0
        dict set config_dict PCIeDMAdone 0

set pcie_clk_pin [get_bd_pins $bd_instance_name/axi_aclk]
set pcie_rst_pin [get_bd_pins proc_sys_rst_pcie/peripheral_aresetn]

dict set config_dict pcie_clk_pin $pcie_clk_pin
dict set config_dict pcie_rst_pin $pcie_rst_pin
...
################################################################
 if { $PCIeJTAG == true } {
  ...
  connect_bd_intf_net [get_bd_intf_pins $bd_instance_name/pcie_cfg_ext] [get_bd_intf_pins debug_bridge_0/pcie3_cfg_ext]
...
connect_bd_intf_net [get_bd_intf_pins $bd_instance_name/M_AXI] -boundary_type upper [get_bd_intf_pins axi_xbar_pcie/S00_AXI]
...
dict set config_dict slv_axi_ninstances $slv_axi_ninstances
